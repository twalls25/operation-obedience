...
- **Admins can't change another admin's status**: `/admin/members/[id]` hides the status form entirely when the target is an admin (shows an explanatory note instead), and `updateMemberStatus` in `src/app/admin/members/actions.ts` refuses the update server-side too — both needed since a UI-only restriction doesn't stop a direct API call. Also extended `016_no_admin_status_changes.sql`'s trigger (the same one from `012_profile_email_and_status.sql` that already blocked self-escalation) so it silently reverts *any* attempt to change role/status on a row that's currently an admin, regardless of who's making the change — the true defense-in-depth layer, since it can't be bypassed by calling Supabase directly. Verified by Tyler on the live site (viewing his own admin profile in `/admin/members` shows the explanatory note instead of the status controls) — Claude couldn't test this one itself, admin-only login-gated flow. **Correction, see the incident below**: this note originally claimed running SQL directly as `postgres` "bypasses both RLS and this trigger" — that was wrong. Superuser status bypasses RLS, but an ordinary trigger fires regardless of role; the trigger itself had to be fixed to actually honor that intent.

## Incident: admin-promotion SQL silently no-op'd (fixed 2026-09-16)
**What happened**: promoted `tyler@ironshepherdsystems.com` to admin via a direct `update public.profiles set role = 'admin' where ...` in the Supabase SQL Editor (`019_promote_real_admin.sql`). It reported success, but the account still showed as `member` afterward.

**Root cause**: the role/status-escalation trigger (`012_profile_email_and_status.sql`, tightened in `016_no_admin_status_changes.sql`) checks `auth.uid()` to decide whether the caller is an admin, and silently reverts `role`/`status` back to their prior values if not. `auth.uid()` reads the current request's JWT — but a raw SQL Editor query has no PostgREST/Auth session at all, so `auth.uid()` is `null` there, which made the trigger's "is the caller an admin?" check always fail and revert the change. The 016 migration's own comment claimed this path "bypasses both RLS and this trigger, same as it bypasses RLS" (see the correction above) — incorrect: superuser status bypasses row-level security, but a plain trigger still fires regardless of role.

**Fix** (`020_fix_admin_bootstrap_trigger.sql`): the trigger function now returns immediately (no-op, change allowed) when `auth.uid() is null`. This is safe rather than a new hole: every real app request goes through PostgREST's RLS, and `profiles`' own update policies already require `auth.uid() = id` or an admin match — both of which are never true when `auth.uid()` is null, so RLS itself already blocks any anonymous/no-session request from ever reaching a row this trigger fires on. A null `auth.uid()` inside the trigger can therefore only mean someone running SQL directly as `postgres`, exactly the case the original design intended to exempt. Re-ran the promotion in the same migration; confirmed by Tyler afterward.

**Lesson**: a migration comment asserting "X bypasses Y" isn't proof it does — this one went unverified for two migrations (012 admin lockout was never actually exercised until this promotion attempt surfaced it). If a future admin lockout/demotion is ever needed via direct SQL, it should now work correctly, but treat any "postgres bypasses this" claim in older notes with suspicion until re-verified.
